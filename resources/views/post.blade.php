...
@endsection
@push('css')
<link rel="stylesheet" href="{{ asset('assets/frontend/css/single_post/styles.css') }}">
<link rel="stylesheet" href="{{ asset('assets/frontend/css/single_post/responsive.css') }}">
<style>
    .favorite_posts{
     color:blue;
    }
					...
					<div class="main-post">
						...
						<div class="blog-post-inner">
							...
							<div class="post-info">
								...
								<div class="left-area">
									<a class="avatar" href="{{ $post->user->username }}"><img src="{{ Storage::disk('public')->url('profile/'.$post->user->image) }}" alt="Profile Image"></a>
								</div>
						...
						<div class="single-post post-style-1">
							...
							<div class="blog-image">
                                <img src="{{ Storage::disk('public')->url('post/'.$randompost->image) }}" alt="Blog Image">
									...
									<a class="avatar" href="#"><img src="{{ Storage::disk('public')->url('profile/'.$randompost->user->image) }}" alt="{{ $randompost->name }}"></a>
							...
							<div class="blog-info">
								...
								<ul class="post-footer">
                                  ...
                                  @else
                                    <a href="javascript:void(0);" onclick="document.getElementById('favorite-post{{ $randompost->id }}').submit();"
                                        class="{{ !Auth::user()->favorite_posts->where('pivot.post_id',$randompost->id)->count()  == 0 ? 'favorite_posts' : ''}}"
                                        ><i class="ion-heart"></i>{{ $randompost->favorite_to_users->count() }}</a>
                                    <form id="favorite-post{{ $randompost->id }}" method="POST" action="{{ route('post.favorite',$randompost->id) }}" style="display:none">
                                    @csrf
                                    </form>
                                  @endguest
